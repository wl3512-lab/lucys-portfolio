/* global React, gsap */
/*
  StaggeredMenu — full-screen staggered slide-in nav.
  Adapted from React Bits (https://reactbits.dev) to this site's no-build globals pattern:
  ESM imports removed (gsap is global, hooks from global React), CSS lives in styles.css,
  recolored for the dark "Lit Archive" brand, and the text wordmark replaces the logo image.
  Items close the menu on click; #about / #contact route through the site's existing
  hash handlers (About overlay, smooth scroll). Exposed as window.StaggeredMenu.
*/
function StaggeredMenu(props) {
  const p = props || {};
  const position = p.position || 'right';
  const colors = p.colors && p.colors.length ? p.colors : ['#4A8FEF', '#C4B0FF'];
  const items = p.items || [];
  const socialItems = p.socialItems || [];
  const displaySocials = p.displaySocials != null ? p.displaySocials : true;
  const displayItemNumbering = p.displayItemNumbering != null ? p.displayItemNumbering : true;
  const logoText = p.logoText || 'lucy liu';
  const menuButtonColor = p.menuButtonColor || '#F6FAFF';
  const openMenuButtonColor = p.openMenuButtonColor || '#F6FAFF';
  const accentColor = p.accentColor || '#72ADFF';
  const changeMenuColorOnOpen = p.changeMenuColorOnOpen != null ? p.changeMenuColorOnOpen : true;
  const closeOnClickAway = p.closeOnClickAway != null ? p.closeOnClickAway : true;
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const plusH = plusHRef.current, plusV = plusVRef.current, icon = iconRef.current, textInner = textInnerRef.current;
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    itemEntranceTweenRef.current && itemEntranceTweenRef.current.kill();
    ...
    if (socialTitle || socialLinks.length) {
      ...
      if (socialLinks.length) {
        tl.to(socialLinks, { y: 0, opacity: 1, duration: 0.55, ease: 'power3.out', stagger: { each: 0.08, from: 'start' },
          onComplete: () => gsap.set(socialLinks, { clearProps: 'opacity' }) }, socialsStart + 0.04);
      }
    }
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current && closeTweenRef.current.kill();
    const offscreen = position === 'left' ? -100 : 100;
  ...
  const animateIcon = useCallback(opening => {
    ...
    if (!icon) return;
    spinTweenRef.current && spinTweenRef.current.kill();
  ...
  const animateColor = useCallback(opening => {
    ...
    if (!btn) return;
    colorTweenRef.current && colorTweenRef.current.kill();
  ...
  const animateText = useCallback(opening => {
    ...
    if (!inner) return;
    textCycleAnimRef.current && textCycleAnimRef.current.kill();
  ...
  const toggleMenu = useCallback(() => {
    ...
    if (target) { p.onMenuOpen && p.onMenuOpen(); playOpen(); }
    else { p.onMenuClose && p.onMenuClose(); playClose(); }
  ...
  const preLayerColors = (function () {
    let arr = colors.slice(0, 4);
  ...
  return (
    <div
      className={(p.className ? p.className + ' ' : '') + 'staggered-menu-wrapper fixed-wrapper'}
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          <a href="/" className="sm-logo-text" aria-label="Lucy Liu, back to top">{logoText}</a>
        </div>
        <button
          ref={toggleBtnRef}
          className="sm-toggle"
